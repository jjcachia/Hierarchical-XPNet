...
#hook for storing intermediate results of some layers
def intermediate_result_hook(intermediate_outputs):
...
class DenseNet_features(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        #storing here the output of each dense block so that it can be used for upconvolution
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                ...
                #register the hook that stores the results of the intermediate transition layers
                trans.register_forward_hook(intermediate_result_hook(self.intermediate_results))
                # print("\nadded hook to: ")
    ...
    def forward(self, x):
        #returning not only the output of the last layer but also the intermediate results of transmission blocks
        #these can then be used for upsampling as in UNET
        self.intermediate_results.clear()
        # print("called output")
        output = self.features(x)
        return output, self.intermediate_results
...
def densenet121_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        # #The below line has been added to remove the last dense block to keep a larger feature map as required by the medical dataset
