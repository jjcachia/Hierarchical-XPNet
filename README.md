...
1. **Shared Feature Extractor & Add-on Layers:** A pre-trained feature extractor and add-on layers serve as the backbone and extract the deep latent feature map from the CT images. It is shared as it is used on both the input, and the learned prototypes (class-representative examples from the training set).
2. **Fixed Gaussian Centered Occurrence Map:** The extracted feature maps are multiplied element-wise by a spatial Gaussian mask. This centers focus directly on the localised nodule, removing focus from irrelevant background lung tissue or imaging features.
3. **Task-Specific Prototype Layer:** The masked feature maps are average-pooled and the input vector is compared against the learned prototype vectors using cosine similarity.
